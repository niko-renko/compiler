...
impl Update for New {
    fn update<'cfg>(
        ...
        let object = cfg.add(Alloc::from(Value::from_raw(2 + field_count)).into());

        let vtable: Place = Static::vtable(class_id).into();
        cfg.add_placed(
            Place::None,
            Set::from(object, Value::from_raw(0).into(), vtable.into()).into(),
        );

        let field_map: Place = Static::fieldmap(class_id).into();
        cfg.add_placed(
            Place::None,
            Set::from(object, Value::from_raw(1).into(), field_map.into()).into(),
        );
